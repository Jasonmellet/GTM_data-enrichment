#!/usr/bin/env python3
"""
Create a list of only the organizations that have been fully enriched with complete data.
"""

import pandas as pd
import os

def create_complete_enriched_list(input_csv, output_csv):
    """Create a list of only fully enriched organizations."""
    ...
    # Filter for organizations that have been fully enriched
    # They should have: email, categories, fit_score, and phone
    df_complete = df[
        (df['email'] != '') & 
        (df['email'] != 'nan') &
        (df['categories'] != '') & 
    ...
    print(f"✅ Fully enriched organizations: {len(df_complete)} records")
    ...
    print(f"💾 Complete enriched list saved to: {output_csv}")
    ...
    print(f"\n🎯 COMPLETE ENRICHED ORGANIZATIONS SUMMARY:")
    ...
    # Show all complete organizations
    print(f"\n🎯 FULLY ENRICHED SUMMER CAMP ORGANIZATIONS:")
...
def main():
    ...
    output_file = "clients/Broadway/outputs/complete_enriched_organizations.csv"
    
    if not os.path.exists(input_file):
        print(f"❌ Input file not found: {input_file}")
        return
    
    print("🚀 Creating Complete Enriched Organizations List for Broadway")
    print("=" * 60)
    
    try:
        df_complete = create_complete_enriched_list(input_file, output_file)
        
        print(f"\n🎉 Success! Complete enriched list created with {len(df_complete)} records")
        print(f"📁 File saved to: {output_file}")
        
        if len(df_complete) == 0:
            print("\n⚠️  WARNING: No organizations have been fully enriched!")
            print("   This suggests the enrichment pipeline needs to be run again.")
        
    except Exception as e:
        print(f"❌ Error creating complete enriched list: {e}")
        import traceback
        traceback.print_exc()
