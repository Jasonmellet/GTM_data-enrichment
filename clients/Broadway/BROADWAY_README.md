...
### **Email Unlocking & Validation Scripts**
...
- **`apollo_enricher.py --comprehensive-discovery`**: Two-phase email discovery strategy ✅
- **`email_discovery.py`** (was `enhanced_email_discovery.py`): pattern-only validation; currently under upgrade
- **`email_catchall_migrator.py`** (was `move_catchall_contacts.py`): move non-validated to catchall ✅
