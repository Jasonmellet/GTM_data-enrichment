...
# Initialize Anthropic client for email generation
...
def update_csv_with_research_and_emails(csv_path, target_company, target_email, research_data, emails, contact_id: str = '', contact_id_header: str | None = None):
    ...
    try:
        ...
        # Update research data (columns AV-AZ)
        research_columns = ['Company Research Summary', 'Contact Research Summary', 'Industry Pain Points', 'Schreiber Opportunity Match', 'Research Quality Score']
        research_values = [
...
def _process_contact_row(df: pd.DataFrame, contact_id_header: str | None, target_contact_id: str):
...
def main():
    ...
    if concurrency > 1:
        ...
        with ThreadPoolExecutor(max_workers=concurrency) as executor:
            future_to_id = {executor.submit(_process_contact_row, df, contact_id_header, target_contact_id): target_contact_id for target_contact_id in target_ids}
        ...
        for target_contact_id in target_ids:
            ok, data = _process_contact_row(df, contact_id_header, target_contact_id)
