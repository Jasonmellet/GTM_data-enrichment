#!/usr/bin/env python3
"""
Generate three separate cold emails with shorter content and different CTAs.
"""
...
def generate_three_emails():
    """Generate three separate cold emails for each contact."""
    print("📧 Generating Three Cold Emails - Testing rows 2, 3, 4")
    print("=" * 70)
    ...
    try:
        ...
        print(f"📊 Headers: {len(data.columns)} columns")
        print(f"📊 First few headers: {list(data.columns[:5])}")
        
        # Process rows 2, 3, and 4 (indices 1, 2, 3)
        print(f"🔍 Generating three emails for rows 2, 3, and 4...")
        ...
        for idx in range(1, 4):  # Process rows 2, 3, 4
            ...
            try:
                # Create prompt for three emails
                prompt = f"""Create three separate cold emails for {first_name} {last_name} at {company_name} ({website}) as a {job_title}.

Company Context: {research_data['company_research'][:300]}...
Contact Context: {research_data['contact_research'][:300]}...
Pain Points: {research_data['pain_points'][:200]}...
Opportunity: {research_data['opportunity_match'][:200]}...

Create THREE separate emails, each with:
1. SUBJECT LINE (max 50 chars, original and non-spammy, NO "Elevate")
2. ICEBREAKER (1-2 sentences, get straight to the point, NO greeting)
3. BODY (2-3 sentences, explain value proposition)
...
CTA: [CTA text]"""
                ...
                if response.choices and len(response.choices) > 0:
                    ...
                    emails = parse_three_emails(content)
                    
                    if emails:
                        # Write emails to columns BA-BM (indices 53-64)
                        print(f"    📝 Writing three emails to sheet (Row {idx + 1}, Columns BA-BM)...")
                        
                        for email_num, email_data in emails.items():
                            for section, text in email_data.items():
                                col_index = get_email_column_index(email_num, section)
                        ...
                        print(f"      ✅ Three emails written to Row {idx + 1}, Columns BA-BM")
                        
                        # Display email summaries
                        print(f"\n📧 Generated Three Emails for {first_name} {last_name}:")
                        print("-" * 50)
                        for email_num, email_data in emails.items():
                            print(f"EMAIL {email_num}:")
                            print(f"  Subject: {email_data.get('subject', 'N/A')}")
                            print(f"  Icebreaker: {email_data.get('icebreaker', 'N/A')[:60]}...")
                            print(f"  Body: {email_data.get('body', 'N/A')[:60]}...")
        ...
        print(f"\n🎉 Three email generation completed for Rows 2, 3, and 4!")
        print(f"🔍 Check your Google Sheet - Rows 2-4, Columns BA-BM should now contain three cold emails each")
